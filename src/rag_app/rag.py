...
class Embeddings:
    ...
    def __init__(self, model_name: str = "sentence-transformers/all-MiniLM-L6-v2"):
        ...
        self._backend = None  # 'sent' or 'tfidf'
        self._model = None
        ...
        if self._model is None:
            if not _HAS_SKLEARN:
                raise ImportError("No embeddings backend available. Install 'sentence-transformers' or 'scikit-learn'.")
...
class RAG:
    def __init__(self,
        ...
        # embeddings wrapper (sentence-transformers if available, else TF-IDF)
        self.embeddings = Embeddings(model_name=EMB_MODEL)
        # text splitter replacement
        self.text_splitter = SimpleTextSplitter(chunk_size=800, chunk_overlap=100)
        self.vs: Optional[Any] = None  # can be SimpleVectorStore or FaissVectorStore

        if os.path.exists(self.index_path):
            # try to load FAISS index first if faiss is available
            try:
                if _HAS_FAISS:
                ...
                self.vs = None
        # --- FIX START: Refit TF-IDF if necessary ---
        # If we loaded an index but are using the TF-IDF backend, the vectorizer 
        # is currently empty (NotFitted). We must refit it on the loaded docs 
        # so it learns the vocabulary again.
        if self.vs is not None and self.embeddings._backend == "tfidf":
            print("INFO: Refitting TF-IDF vectorizer on loaded documents...")
            # We extract the text from the loaded vector store documents
            texts = [d.page_content for d in self.vs.documents]
            # Calling embed_documents triggers fit_transform internally for TF-IDF
    ...
    def build_index(self):
        ...
        Splits each document using SimpleTextSplitter, embeds chunks, and saves a local index.
        """
        docs = self.load_folder_docs()
        if not docs:
            raise RuntimeError(f"No documents found in {self.docs_folder} to build index.")

        # split and create chunked Document objects
        ...
        # choose FAISS if available, else simple store
        if _HAS_FAISS:
    ...
    def get_context(self, query: str, k: int = 4) -> str:
        ...
        qvec = self.embeddings.embed_query(query)  # shape (D,)

        # If using FaissVectorStore, use its search; if SimpleVectorStore, use its search.
        if _HAS_FAISS and isinstance(self.vs, FaissVectorStore):
